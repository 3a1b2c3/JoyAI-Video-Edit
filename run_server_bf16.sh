#!/bin/bash
# Start JoyAI-Video-Edit server with FP8 disabled (bf16 attention/MLP
# throughout -- deploy/run_server.sh otherwise defaults JOYOMNI_FP8_IMG/TXT
# to 1 if unset). Low-VRAM mode is decided by run_server_best.sh's GPU
# auto-detection, not forced here.
set -uo pipefail
